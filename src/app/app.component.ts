import {Component, ViewChild} from "@angular/core";
import {Platform, MenuController, Nav} from "ionic-angular";
import {StatusBar, Splashscreen} from "ionic-native";
import {HelloIonicPage} from "../pages/hello-ionic/hello-ionic";
import {SessionsPage} from "../pages/sessions/sessions";
import {PresentateursPage} from "../pages/presentateurs/presentateurs";
import {AboutPhonePage} from "../pages/about-phone/about-phone";
import {ParcoursPage} from "../pages/parcours/parcours";


@Component({
  templateUrl: 'app.html'
})
export class MyApp {
  @ViewChild(Nav) nav: Nav;

  // make HelloIonicPage the root (or first) page
  rootPage: any = HelloIonicPage;
  pages: Array<{title: string, component: any}>;

  constructor(
    public platform: Platform,
    public menu: MenuController
  ) {
    this.initializeApp();

    // set our app's pages
    this.pages = [
      { title: 'Accueil', component: HelloIonicPage },
       { title: 'Les Présentateurs', component: PresentateursPage },
      { title: 'Les Sessions', component: SessionsPage },
      { title: 'Mon Parcours', component: ParcoursPage},
      { title: 'A propos du Téléphone', component: AboutPhonePage }
    ];
  }

  initializeApp() {
    this.platform.ready().then(() => {
      // Okay, so the platform is ready and our plugins are available.
      // Here you can do any higher level native things you might need.
      StatusBar.styleDefault();
      Splashscreen.hide();
    });
  }

  openPage(page) {
    // close the menu when clicking a link from the menu
    this.menu.close();
    // navigate to the new page if it is not the current page
    this.nav.push(page.component);
  }
}
